...
export async function generateDisputeLetter(action: Record<string, unknown>) {
  ...
  const systemPrompt = `You are a professional credit dispute letter writer for JECI Credit.
You write formal, legally-aware dispute letters to credit bureaus.
Follow FCRA guidelines. Be concise, firm, and professional.
Do NOT use aggressive or threatening language.
Always request investigation within 30 days per FCRA.
...
Generate a complete, professional dispute letter ready to send.`;
  ...
  if (dispute_id) {
    await supabaseAdmin
      .from('disputes')
      .update({ letter_content: letterContent, status: 'ready_to_send' })
      .eq('id', dispute_id);
  }
