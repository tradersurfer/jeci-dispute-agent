...
-- ── Paid Sessions ─────────────────────────────────────────────
-- Tracks Stripe-confirmed payments for dashboard access gating
CREATE TABLE IF NOT EXISTS paid_sessions (
  ...
  customer_email TEXT,
  customer_name  TEXT,
  plan           TEXT,                   -- 'JECI Scan' | 'JECI Sweep' | 'JECI Repair' | 'JECI Boost'
  price_id       TEXT,
...
-- Create the dispute-packages bucket (run once, or use Supabase dashboard)
